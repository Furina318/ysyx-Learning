...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd)=s->pc+4;s->dnpc+=imm);//jal指令中是当前pc值加上符号位拓展的offset，而dnpc已经指向下一条指令，所以需要减去4。
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", ret    , I, R(rd)=s->pc+4;s->dnpc=(src1+imm) & ~1); // ret 是 jalr x0, 0(x1)
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1+imm,4,src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1+imm,2,src2));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd)=Mr(src1+imm,4));
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->dnpc+=src1>=src2 ? imm:0);
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc+=src1==src2 ? imm:0);//beqz可视为beq rs1, x0, offset.  
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, {uint32_t shamt = src2 & 0x1F;R(rd) = (int32_t)src1 >> shamt;});
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, {uint32_t shamt = src2 & 0x1F;R(rd) = (uint32_t)src1 >> shamt;});
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc+=src1<src2 ? imm:0);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, s->dnpc+=(uint32_t)src1<(uint32_t)src2 ? imm:0);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc+=src1>=src2 ? imm:0);
