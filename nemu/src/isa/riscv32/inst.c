...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", ret    , I, R(rd)=s->pc+4;s->dnpc=((src1+imm) & ~1)-4); // ret 是 jalr x0, 0(x1)
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1+imm,4,src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1+imm,2,src2));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd)=Mr(src1+imm,4));
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->dnpc+=src1>=src2 ? imm-4:0);
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc+=src1==src2 ? imm-4:0);//beqz可视为beq rs1, x0, offset.  
