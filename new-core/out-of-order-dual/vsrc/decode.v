...
module decode (
    ...
    output reg  [  `DU_BUS_WD-1:0] du_bus       ,
    output reg  [`DU_TO_RE_WD-1:0] du_to_re_bus
);
    wire         gpr_we;
    ...
    assign TYPE_I = (opcode_06_00 == 7'h13) | is_read | inst_jarl | inst_csrrw | inst_csrrs | inst_fence_i; 
    ...
    wire inst_ctrl = inst_jarl | inst_jal | inst_beq | inst_bne | inst_bge | inst_bgeu | inst_blt | inst_bltu | inst_fence_i;
    wire inst_lsu  = is_read | is_write;
    wire inst_mdu  = |mdu_op;
    // 一周期可以计算出来的指令
    wire inst_quick = inst_add   | inst_and  | inst_or    | inst_xor     | inst_sll  |
                      inst_sub   | inst_slt  | inst_sltu  | inst_addi    | inst_andi | 
                      inst_jarl  | inst_slti | inst_sltiu | inst_slli    | inst_sra  | 
                      inst_srai  | inst_srl  | inst_srli  | inst_ori     | inst_xori |
                      inst_auipc | inst_lui  | inst_jal   | inst_fence_i;
