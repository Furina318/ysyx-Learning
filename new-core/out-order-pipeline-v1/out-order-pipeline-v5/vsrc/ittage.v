...
//
// 功能: 间接分支预测，使用 TAGE 算法变体。
//   4 张预测表 (T1~T4)，历史长度递增。
//   最长匹配 + 最高 useful 优先提供预测。
//   全局历史寄存器 (GHR) 索引各表。
//   双发射训练: slot 1 (older) + slot 2 (younger) 训练端口。
// =============================================================================
module ittage #(
    ...
    // ==================== slot 2 训练索引/标签 (使用 slot 1 更新后的 GHR) ====================
    // slot 1 的 GHR 更新中间态
    wire [GHR_WIDTH-1:0] ghr_after_slot1;
    assign ghr_after_slot1 = (update_en1 && br_taken1)  ? {ghr[GHR_WIDTH-2:0], 1'b1} :
                             (update_en1 && !br_taken1) ? {ghr[GHR_WIDTH-2:0], 1'b0} : ghr;

    wire [31:0] _ut1_idx_2 = update_en2 ? hash_pc_ghr(update_pc2, ghr_after_slot1, 4)  : 0;
    wire [31:0] _ut2_idx_2 = update_en2 ? hash_pc_ghr(update_pc2, ghr_after_slot1, 8)  : 0;
    wire [31:0] _ut3_idx_2 = update_en2 ? hash_pc_ghr(update_pc2, ghr_after_slot1, 16) : 0;
    wire [31:0] _ut4_idx_2 = update_en2 ? hash_pc_ghr(update_pc2, ghr_after_slot1, 32) : 0;
    ...
    wire [31:0] _ut1_tag_2 = update_en2 ? hash_pc_ghr(update_pc2, ghr_after_slot1, 8)  : 0;
    wire [31:0] _ut2_tag_2 = update_en2 ? hash_pc_ghr(update_pc2, ghr_after_slot1, 16) : 0;
    wire [31:0] _ut3_tag_2 = update_en2 ? hash_pc_ghr(update_pc2, ghr_after_slot1, 32) : 0;
    wire [31:0] _ut4_tag_2 = update_en2 ? hash_pc_ghr(update_pc2, ghr_after_slot1, 64) : 0;
    ...
    // ==================== slot 1 训练 provider 重计算 ====================
    // 训练时用 update_pc1 和当前表内容重新查找 provider
    wire hit_train_t4_1 = valid_t4[update_t4_idx_1] && (tag_t4[update_t4_idx_1] == update_t4_tag_1);
    wire hit_train_t3_1 = valid_t3[update_t3_idx_1] && (tag_t3[update_t3_idx_1] == update_t3_tag_1);
    wire hit_train_t2_1 = valid_t2[update_t2_idx_1] && (tag_t2[update_t2_idx_1] == update_t2_tag_1);
    wire hit_train_t1_1 = valid_t1[update_t1_idx_1] && (tag_t1[update_t1_idx_1] == update_t1_tag_1);

    wire train_provider_valid_1 = hit_train_t4_1 | hit_train_t3_1 | hit_train_t2_1 | hit_train_t1_1;
    wire [3:0] train_provider_table_1 =
        hit_train_t4_1 ? 4 : hit_train_t3_1 ? 3 : hit_train_t2_1 ? 2 : hit_train_t1_1 ? 1 : 0;

    // ==================== slot 2 训练 provider 重计算 ====================
    wire hit_train_t4_2 = valid_t4[update_t4_idx_2] && (tag_t4[update_t4_idx_2] == update_t4_tag_2);
    wire hit_train_t3_2 = valid_t3[update_t3_idx_2] && (tag_t3[update_t3_idx_2] == update_t3_tag_2);
    wire hit_train_t2_2 = valid_t2[update_t2_idx_2] && (tag_t2[update_t2_idx_2] == update_t2_tag_2);
    wire hit_train_t1_2 = valid_t1[update_t1_idx_2] && (tag_t1[update_t1_idx_2] == update_t1_tag_2);

    wire train_provider_valid_2 = hit_train_t4_2 | hit_train_t3_2 | hit_train_t2_2 | hit_train_t1_2;
    wire [3:0] train_provider_table_2 =
        hit_train_t4_2 ? 4 : hit_train_t3_2 ? 3 : hit_train_t2_2 ? 2 : hit_train_t1_2 ? 1 : 0;

    // ==================== 写冲突检测 ====================
    wire conflict_t1 = update_en1 && update_en2 && (update_t1_idx_1 == update_t1_idx_2);
    wire conflict_t2 = update_en1 && update_en2 && (update_t2_idx_1 == update_t2_idx_2);
    wire conflict_t3 = update_en1 && update_en2 && (update_t3_idx_1 == update_t3_idx_2);
    wire conflict_t4 = update_en1 && update_en2 && (update_t4_idx_1 == update_t4_idx_2);
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            // ============ GHR 顺序更新 (slot 1 先, slot 2 后) ============
            if (update_en2 && br_taken2)
                ghr <= {ghr_after_slot1[GHR_WIDTH-2:0], 1'b1};
            else if (update_en2 && !br_taken2)
                ghr <= {ghr_after_slot1[GHR_WIDTH-2:0], 1'b0};
            else if (update_en1 && br_taken1)
            ...
            // ============ slot 1 表训练 ============
            if (update_en1) begin
                if (train_provider_valid_1) begin
                    case (train_provider_table_1)
                        1: begin
            ...
            // ============ slot 2 表训练 (冲突时覆盖 slot 1) ============
            if (update_en2) begin
                if (train_provider_valid_2) begin
                    case (train_provider_table_2)
                        1: begin
                            if (!conflict_t1 || train_provider_table_1 != 1) begin
                    ...
                    if (!valid_t4[update_t4_idx_2] || useful_t4[update_t4_idx_2] == 0) begin
                        ...
                        target_t3[update_t3_idx_2] <= real_target2;
                        useful_t3[update_t3_idx_2] <= br_taken2 ? 2'b10 : 2'b01;
                        ...
                        target_t2[update_t2_idx_2] <= real_target2;
                        useful_t2[update_t2_idx_2] <= br_taken2 ? 2'b01 : 2'b00;
