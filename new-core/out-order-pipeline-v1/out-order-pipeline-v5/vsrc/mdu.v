...
always @(posedge clk) begin
    if (rst) begin
        ...
        mdu_src2_r   <= 32'd0;
        latched_flag <= 1'b0;
    end else if (mdu_en && (cnt_n == 6'd0) && !muling && !diving && !mdu_done && !latched_flag) begin
        // 当收到使能信号且模块空闲时，锁存输入数据
        mdu_op_r     <= mdu_op;
        mdu_src1_r   <= mdu_src1;
        mdu_src2_r   <= mdu_src2;
        latched_flag <= 1'b1; // 产生一个周期的有效脉冲，用于启动后续计算
    end else begin
        latched_flag <= 1'b0;
    end
end
...
assign rem_result    = div_zero_flag ? mdu_src1_r : src1_sign ? ~result[63:32] + 1 : result[63:32];
...
assign multiplicand_ext = is_signed_mul_a ? {{36{mdu_src1_r[31]}}, mdu_src1_r}
                                          : {36'd0, mdu_src1_r};
assign multiplier_ext = is_signed_mul_b ? {{2{mdu_src2_r[31]}}, mdu_src2_r, 1'b0}
...
always @(posedge clk) begin
    if (rst) begin
        cnt_n    <= 6'd0;
        mdu_done <= 1'b0;
        muling   <= 1'b0; // 建议在rst中增加这两项以防未定态向锁存模块传播
        diving   <= 1'b0;
    end else begin
        mdu_done <= 1'b0;
        
        // 使用锁存完成标志位 latched_flag 来作为状态机的启动信号
        if (latched_flag & mul_en & cnt_n == 6'd0) begin
            muling <= 1'b1;
            cnt_n  <= cnt_n + 6'b1;
        ...
        // 使用锁存完成标志位 latched_flag 触发除法
        if (latched_flag & div_en & cnt_n == 6'd0) begin
