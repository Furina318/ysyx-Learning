module bpu #(
    ...
    input  wire [DATA_WIDTH-1:0]    bru_pred_dnpc_2  ,  
    ...
    wire                   bru_mispredict_2 = bru_dnpc_valid_2 && (bru_dnpc_2 != bru_pred_dnpc_2);
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (bpu_en_1) begin
                ...
                meta_dir[pred_meta_idx_1]            <= cond_pred_taken_1;
            ...
            // slot 2
            if (bpu_en_2) begin
                meta_ghr[pred_meta_idx_2]            <= ghr_s2;
                meta_dir[pred_meta_idx_2]            <= cond_pred_taken_2;
            ...
            if (mispredict_active) begin
                ...
                if (mispredict_from_slot1) begin
                    ...
                    // slot2 的 GHR 快照是“slot1 预测前”的 GHR, 若 slot1 是条件分支
                    // 且未误预测, 需要先补上 slot1 的真实方向。
                    if (bru_dnpc_valid_1 && !bru_is_jal_1 && !bru_is_jalr_1) begin
                        if (bru_is_jal_2 || bru_is_jalr_2) begin
                            ghr <= {hist_ghr_2[GHR_WIDTH-2:0], bru_taken_1};
                        end else begin
                            ghr <= {{hist_ghr_2[GHR_WIDTH-3:0], bru_taken_1}, bru_taken_2};
                        end
                    end else begin
                        if (bru_is_jal_2 || bru_is_jalr_2) begin
                            ghr <= hist_ghr_2;
            ...
            if (bru_dnpc_valid_2 && !ftb_conflict) begin
                if (update_ftb_hit_2) begin
                    ...
                    // Fix: 目标为 0 不写 (错路径可能算出 target=0, 写进 FTB 无意义)
                    if ((bru_taken_2 || bru_is_jal_2 || bru_is_jalr_2) && (bru_dnpc_2 != 32'h0)) begin
            ...
            if (bru_dnpc_valid_2) begin
                bp_all_cnt <= bp_all_cnt + 64'd1;
                if (bru_dnpc_2 != bru_pred_dnpc_2)
                    bp_all_misp_cnt <= bp_all_misp_cnt + 64'd1;
                if (!bru_is_jal_2 && !bru_is_jalr_2) begin
                    bp_cond_cnt <= bp_cond_cnt + 64'd1;
                    if (bru_dnpc_2 != bru_pred_dnpc_2)
