module icache #(
    ...
    // 统计
    reg [63:0] icache_access_cnt /*verilator public*/;
    reg [63:0] icache_hit_cnt    /*verilator public*/;
    reg [63:0] icache_miss_cnt   /*verilator public*/;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            axi_rready <= 1'b0;
            axi_arvalid <= 1'b0;
            icache_access_cnt <= 64'd0;
            icache_hit_cnt    <= 64'd0;
            icache_miss_cnt   <= 64'd0;
        end
        else begin
            if (is_fencei) begin
                for (idx = 0; idx < NUM_BLOCKS; idx = idx + 1) begin
            ...
            case (state)
                IDLE: begin
                    ...
                    // count statistics
                    if (req_valid && !is_fencei) begin
                        icache_access_cnt <= icache_access_cnt + 64'd1;
                        if (hit) begin
