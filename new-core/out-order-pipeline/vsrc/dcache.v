...
reg        device_done;
reg [31:0] pend_addr;    // 最近一次 device 请求地址, 用于检测请求是否变化
...
assign dcache_wdata = lsu_wdata | (data_array[index][32*offset/4 +: 32] & ~wdata_mask);
assign lsu_rdata    = is_mem    ? data_array[index][offset*8 +: DATA_WIDTH] : device_rdata;
// Fix: device_done 置位的那个周期 state 仍是 MEM_BUSY (state 下一拍才变回 IDLE),
// 因此不能以 (state==IDLE) 作为判据 (start_new 永远不满足, 防不住地址变化).
// 只要当前请求地址与本次实际读到的地址 (pend_addr) 不一致, 就必须禁止
// lsu_ready 用旧 device_rdata 响应新地址的 Load (否则会返回上一次读的脏数据).
// 典型场景: 慢路径 Load 被 flush, 但 dcache 的 AXI 读尚未完成; 新请求地址
// 已变化时, dcache_ready 会错误地把旧数据交给新 Load.
// 连续同地址访问 (如 LSU 持续等待数据) 不受影响.
wire addr_changed = (lsu_addr != pend_addr);
wire device_ready = device_done && !addr_changed;
assign lsu_ready  = lsu_valid ? (dcache_hit | device_ready) : ~fence_ing;
    ...
    if (rst) begin
        ...
        case (state)
            IDLE: begin
                ...
                end else if (lsu_valid && !dcache_hit) begin
                    // 记录请求地址, 用于检测后续请求是否与当前相同
                    if (!is_mem) pend_addr <= lsu_addr;
                    if ((dirty_array[index] && is_mem) | (~is_mem && lsu_we)) begin
                        awaddr  <= is_mem ? {tag_array[index], index, {OFFSET_WIDTH{1'b0}}} : lsu_addr;
                        awvalid <= 1'b1;
                        wdata   <= is_mem ? data_array[index][burst_cnt*32 +: 32] : lsu_wdata;
                        wstrb   <= is_mem ? 4'hf : lsu_wstrb;
                        wvalid  <= 1'b1;
                        ...
                        araddr  <= is_mem ? block_addr : lsu_addr;
                        arvalid <= 1'b1;
                        arid    <= 4'h0;
