...
extern "C" void debug_s3_write(int val, int pc_val) {
    fprintf(stderr, "[S3_WRITE] pc=0x%08x value=0x%08x time=%ld\n", pc_val, val, main_time);
}
extern "C" void ebreak(int station, int inst) {
    if(main_time>=start_time){
        ...
        npc_state.halt_pc = top->rootp->ysyxSoCFull__DOT__asic__DOT__cpu__DOT__cpu__DOT__ifu_pc;
        #else
        npc_state.halt_pc = top->rootp->ysyx_25010030_npc__DOT__cpu__DOT__ifu_pc;
