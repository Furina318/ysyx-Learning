...
always @(*) begin
    case (current_master)
        IDLE         : next_master = (lsu_arvalid | lsu_awvalid) ? LSU_ACCESS  :
                                     (ifu_arvalid & fence_done ) ? IFU_ACCESS    : IDLE;
        IFU_ACCESS   : next_master = (ifu_rvalid && ifu_rready && ifu_rlast) ? IDLE : IFU_ACCESS;
        LSU_ACCESS   : next_master = ((lsu_rvalid && lsu_rready && lsu_rlast) | (clint_rvalid & clint_rready) | 
                                     (io_master_bvalid & io_master_bready)) ? IDLE : LSU_ACCESS;
...
assign lsu_rid           = (current_master == LSU_ACCESS) ? is_clint_addr ? 'b0           : io_master_rid     : 'b0;
