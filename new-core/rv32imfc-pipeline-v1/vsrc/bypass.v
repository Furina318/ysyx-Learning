module bypass #(
    ...
    parameter ZERO_READONLY     = 1'b1   // 1: 寻址 0 恒读 0 (x0); 0: 允许旁路写 0 (f0)
) (
    input  wire         clk              ,
    input  wire         rst              ,
    // from exu
...
reg [31:0] bypass_src3;

always @(*) begin
    bypass_src1 = rf_rs1_data;
    if ((bypass_rd[0] == rs1_addr) && ((rs1_addr != 5'd0) | !ZERO_READONLY) && bypass_valid[0]) begin
        bypass_src1 = bypass_data[0];
    end
    else if ((bypass_rd[1] == rs1_addr) && ((rs1_addr != 5'd0) | !ZERO_READONLY) && bypass_valid[1]) begin
        bypass_src1 = bypass_data[1];
    end
    else if ((bypass_rd[2] == rs1_addr) && ((rs1_addr != 5'd0) | !ZERO_READONLY) && bypass_valid[2]) begin
...
always @(*) begin
    ...
    if ((bypass_rd[0] == rs2_addr) && ((rs2_addr != 5'd0) | !ZERO_READONLY) && bypass_valid[0]) begin
        bypass_src2 = bypass_data[0];
    end
    else if ((bypass_rd[1] == rs2_addr) && ((rs2_addr != 5'd0) | !ZERO_READONLY) && bypass_valid[1]) begin
        bypass_src2 = bypass_data[1];
    end
    else if ((bypass_rd[2] == rs2_addr) && ((rs2_addr != 5'd0) | !ZERO_READONLY) && bypass_valid[2]) begin
        bypass_src2 = bypass_data[2];
    end
end

always @(*) begin
    bypass_src3 = rf_rs3_data;
    if ((bypass_rd[0] == rs3_addr) && ((rs3_addr != 5'd0) | !ZERO_READONLY) && bypass_valid[0]) begin
        bypass_src3 = bypass_data[0];
    end
    else if ((bypass_rd[1] == rs3_addr) && ((rs3_addr != 5'd0) | !ZERO_READONLY) && bypass_valid[1]) begin
        bypass_src3 = bypass_data[1];
    end
    else if ((bypass_rd[2] == rs3_addr) && ((rs3_addr != 5'd0) | !ZERO_READONLY) && bypass_valid[2]) begin
        bypass_src3 = bypass_data[2];
    end
end
...
assign bypass_rs3_data  = bypass_src3;
