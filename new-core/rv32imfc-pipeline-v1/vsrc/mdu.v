...
assign rem_result    = div_zero_flag ? mdu_src1 : src1_sign ? ~result[63:32] + 1 : result[63:32];
...
always @(posedge clk) begin
    if (rst) begin
        ...
        mdu_done <= 1'b0;
        if (~mdu_done & mul_en & pipeline_ready & cnt_n == 6'd0) begin
            muling <= 1'b1;
            cnt_n  <= cnt_n + 6'b1;
        ...
        if (~mdu_done & div_en & pipeline_ready & cnt_n == 6'd0) begin
