...
/* verilator lint_off SYNCASYNCNET */ // reset 同时用于同步复位与模块内异步使用
...
`endif
    ...
    wire [            31:0] ifu_pc;
    /* verilator lint_off UNDRIVEN */ // ifu_inst: difftest 预留, 当前 ifu 未驱动
    wire [            31:0] ifu_inst;
    ...
    wire [            4:0]  exu_fp_rd_w_bypass;
    wire [           31:0]  exu_fp_rd_w_bypass_data;
    wire                    exu_fp_rd_w_bypass_en;
    wire                    exu_is_read;
    wire                    exu_is_fencei;
    ...
    // fp: fregfile 读端口(wbu 内) + fp bypass
    wire [31:0] rf_frs1_data;
    wire [31:0] rf_frs2_data;
    wire [31:0] rf_frs3_data;
    wire [ 2:0] fp_frm;
    wire [ 4:0] fp_fflags;

    wire [31:0] bypass_rs1_data;
    wire [31:0] bypass_rs2_data;
    wire [31:0] gpr_bypass_rs3_data;
    wire [31:0] fp_bypass_rs1_data;
    wire [31:0] fp_bypass_rs2_data;
    wire [31:0] fp_bypass_rs3_data;
    ...
    always @(posedge clock) begin
        ...
        else if (!exu_is_fencei              ) fencei_killed <= 1'b0;
    end
    wire        is_fencei = exu_is_fencei & !exu_flush_en & !fencei_killed; // 避免出现因为跳转错误而引起的fence操作
    ...
    bypass u_bypass (
        ...
        .bypass_rs3_data  (gpr_bypass_rs3_data)
    );

    // fpr bypass: 与 gpr 同构, 前递最近写回的 FPU 结果 (f0 为普通寄存器)
    bypass #(.ZERO_READONLY(0)) u_fp_bypass (
        .clk              (clock                  ),
        .rst              (reset                  ),
        .idu_valid        (idu_valid              ),
        .exu_ready        (exu_ready              ),
        ...
        .rs2_addr         (exu_rs2                ),
        .rs3_addr         (exu_rs3                ),
        .rd_w_bypass      (exu_fp_rd_w_bypass      ),
        .rd_w_bypass_data (exu_fp_rd_w_bypass_data ),
        .rd_w_bypass_en   (exu_fp_rd_w_bypass_en  ),
        .rf_rs1_data      (rf_frs1_data           ),
        .rf_rs2_data      (rf_frs2_data           ),
        .rf_rs3_data      (rf_frs3_data           ),
        .bypass_rs1_data  (fp_bypass_rs1_data     ),
        .bypass_rs2_data  (fp_bypass_rs2_data     ),
        .bypass_rs3_data  (fp_bypass_rs3_data     )
    );
    ...
    exu u_exu (
        ...
        .is_read          (exu_is_read        ),
        .rs3_addr         (exu_rs3            ),
        .fp_rd_w_bypass      (exu_fp_rd_w_bypass      ),
        .fp_rd_w_bypass_data (exu_fp_rd_w_bypass_data ),
        .fp_rd_w_bypass_en   (exu_fp_rd_w_bypass_en   ),
        .fp_rs1_data       (fp_bypass_rs1_data ),
        .fp_rs2_data       (fp_bypass_rs2_data ),
        .fp_rs3_data       (fp_bypass_rs3_data ),
        .fp_frm            (fp_frm             ),
        .bru_pc           (bru_pc             ),
        .bru_dnpc         (bru_dnpc           ),
        .bru_dnpc_valid   (bru_dnpc_valid     ),
    ...
    wbu wbu (
        ...
        .rf_rs1_data  (rf_rs1_data   ),
        .rf_rs2_data  (rf_rs2_data   ),
        .rf_frs1_data (rf_frs1_data  ),
        .rf_frs2_data (rf_frs2_data  ),
        .rf_frs3_data (rf_frs3_data  ),
        .fp_frm       (fp_frm        ),
        .fp_fflags    (fp_fflags     )
    );
