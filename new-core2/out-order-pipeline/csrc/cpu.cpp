...
// 检测分支预测 miss 并写入 btrace 日志。
// 判定与 bpu.v 中 mispredict 完全一致: bru_dnpc_valid 且真实目标 != 预测目标。
// 此时 bru_pc/bru_dnpc/bru_pred_dnpc 锁存的是该分支指令的信息。
#ifdef CONFIG_BTRACE
static void btrace_sample_miss(void) {
#ifdef YSYXSOC
    auto *cpu = top->rootp->ysyxSoCFull__DOT__asic__DOT__cpu__DOT__cpu;
    uint32_t bpc        = cpu->bru_pc;
    uint32_t pred_dnpc  = cpu->bru_pred_dnpc;
    uint32_t dnpc       = cpu->bru_dnpc;
    bool     valid      = cpu->bru_dnpc_valid;
    bool     is_c_inst  = cpu->bru_is_c_inst;
    bool     taken      = cpu->bru_taken;
    bool     is_jal     = cpu->bru_is_jal;
    bool     is_jalr    = cpu->bru_is_jalr;
    bool     is_call    = cpu->bru_is_call;
    bool     is_ret     = cpu->bru_is_ret;
    bool     is_indirect= cpu->bru_is_indirect;
#else
    auto *cpu = top->rootp->ysyx_25010030_npc->cpu;
    uint32_t bpc        = cpu->bru_pc;
    uint32_t pred_dnpc  = cpu->bru_pred_dnpc;
    uint32_t dnpc       = cpu->bru_dnpc;
    bool     valid      = cpu->bru_dnpc_valid;
    ...
    bool     is_indirect= cpu->bru_is_indirect;
#endif
    if (!valid || dnpc == pred_dnpc) return;
    // 指令本身在 guest 内存中只读, 直接按 pc 取 (压缩指令 2 字节)
    uint32_t binst = is_c_inst ? (uint32_t)pmem_r(bpc, 2) : (uint32_t)pmem_r(bpc, 4);
    btrace_log(cycle_sum, bpc, binst, pred_dnpc, dnpc,
...
static void execute_once() {
    ...
    do{
      ...
      #endif
cycle_sum++;
        IFDEF(CONFIG_BTRACE, btrace_sample_miss());
...
static void execute(uint64_t n) {
    ...
    if(npc_state.state==NPC_END || npc_state.state==NPC_ABORT){
        ...
        IFDEF(CONFIG_BTRACE,close_btrace());
    }
}
