...
wire error_read = io_master_arvalid && !is_sram_addr && !is_uart_addr;
wire error_write = io_master_awvalid && !is_sram_addr && !is_uart_addr;
always @(posedge clk) begin
    if (error_read) begin
        $display("AXI_Bridge: Error - Invalid read address: 0x%08x", io_master_araddr);
    end
    if (error_write) begin
        $display("AXI_Bridge: Error - Invalid write address: 0x%08x", io_master_awaddr);
    end
end
