...
module exu_slot1 #(
    ...
    // Fix: 该指令发射时的预测目标, 必须与 bru_dnpc 同拍锁存 (同一指令)。
    // 若用组合的当拍 bpu_dnpc, BPU 比较 bru_dnpc_1 (寄存旧指令目标) 与
    // bru_pred_dnpc_1 (当拍新指令预测) 不同步 → 误报 mispredict。
    output reg  [           31: 0]  bru_pred_dnpc    ,
    ...
    // 该指令预测时刻的元数据快照 (随指令携带, 供 BPU 训练/恢复)
    output reg  [           31: 0]  bru_ghr          ,
    output reg  [            2: 0]  bru_provider     ,
    output reg                      bru_alt_taken    ,
    output reg                      bru_dir          ,
    output reg  [            3: 0]  bru_ras_ptr      ,
    output reg  [            4: 0]  bru_ras_cnt      ,
    ...
    wire        bpu_alt;
    wire        bpu_dir;
    wire [ 3:0] bpu_ras_ptr;
    wire [ 4:0] bpu_ras_cnt;
    wire [ 1:0] lw_lh_lb;
    wire [ 1:0] sw_sh_sb;
    wire        is_load_sign;
    wire        inst_fence_i;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (is_ctrl && done && issue_valid) begin
                bru_pc          <= bru_pc_c;
                bru_dnpc        <= bru_dnpc_c;
                // Fix: 与 bru_dnpc 同拍锁存 (同一指令), 保证 BPU 比较同步
                bru_pred_dnpc   <= bpu_dnpc;
                ...
                // 与 bru_dnpc 同拍锁存该指令的预测时刻元数据快照
                bru_ghr         <= bpu_ghr;
                bru_provider    <= bpu_prov;
                bru_alt_taken   <= bpu_alt;
                bru_dir         <= bpu_dir;
                bru_ras_ptr     <= bpu_ras_ptr;
                bru_ras_cnt     <= bpu_ras_cnt;
            end else begin
                bru_dnpc_valid  <= 1'b0;
                bru_taken       <= 1'b0;
                bru_is_call     <= 1'b0;
