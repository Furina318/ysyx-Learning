...
module ifu (
    ...
    // Pulse when a new fetch window is requested by the front end.  Keeping
    // this separate from icache_valid lets the cache account a miss exactly
    // once (the request remains outstanding while the line is filled).
    ...
    // 每条指令的预测时刻元数据快照 (随指令携带, 供后端训练/恢复, 避免 pc 索引覆盖竞态)
    input  wire [31: 0] bpu_ghr1       ,
    input  wire [31: 0] bpu_ghr2       ,
    input  wire [ 2: 0] bpu_prov1      ,
    input  wire [ 2: 0] bpu_prov2      ,
    input  wire         bpu_alt1       ,
    input  wire         bpu_alt2       ,
    input  wire         bpu_dir1       ,
    input  wire         bpu_dir2       ,
    input  wire [ 3: 0] bpu_ras_ptr1   ,
    input  wire [ 3: 0] bpu_ras_ptr2   ,
    input  wire [ 4: 0] bpu_ras_cnt1   ,
    input  wire [ 4: 0] bpu_ras_cnt2   ,
    ...
    // ─── ifu_valid ───
    wire base_valid = inst_ready & ~exu_flush_en & ~flush_stall;
    ...
    assign pc_updata = inst_ready & ibu_ready & ~flush_stall;

    wire [31:0] pre_dnpc1 = is_jal_1 ? jal_target1 : dnpc_slot1;
    wire [31:0] pre_dnpc2 = is_jal_2 ? jal_target2 : dnpc_slot2;

    assign if_to_ib_bus1 = {final_s1_pc, pre_dnpc1,
                            bpu_ghr1, bpu_prov1, bpu_alt1, bpu_dir1,
                            bpu_ras_ptr1, bpu_ras_cnt1,
                            final_s1_inst,
                            i_call_1, i_ret_1, i_jal_1, i_jalr_1, i_ind_1, final_s1_is_c};
    assign if_to_ib_bus2 = {final_s2_pc, pre_dnpc2,
                            bpu_ghr2, bpu_prov2, bpu_alt2, bpu_dir2,
                            bpu_ras_ptr2, bpu_ras_cnt2,
                            final_s2_inst,
                            i_call_2, i_ret_2, i_jal_2, i_jalr_2, i_ind_2, final_s2_is_c};

    reg flush_stall;          // flush 后等待 icache 对新的 pc 有效再放行，避免旧窗口指令混入
    reg        stat_icache_outstanding;

    // 用于统计
    assign icache_req = ~stat_icache_outstanding & ~exu_flush_en;

    always @(posedge clk) begin
        if (rst) begin
            flush_stall          <= 1'b0;
            stat_icache_outstanding <= 1'b0;
        end else begin
            if (flush_pulse) begin
                flush_stall           <= 1'b1;
                // Force a fresh accounting event for the redirected fetch.
                stat_icache_outstanding <= 1'b0;
            end else if (flush_stall && icache_valid && (pc == exu_flush_dnpc)) begin
                flush_stall <= 1'b0;
            end
            if (icache_req) begin
                stat_icache_outstanding <= 1'b1;
            end else if (stat_icache_outstanding && pc_updata) begin
                stat_icache_outstanding <= 1'b0;
            end
        end
    end
