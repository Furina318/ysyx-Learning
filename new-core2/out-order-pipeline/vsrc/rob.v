...
module rob #(
    ...
    // 供 load buffer 判断 MMIO load 是否已到 ROB 最老 (非推测)
    output wire                         rob_oldest_valid,
    output wire [     ID_WIDTH-1:0]     rob_oldest_id
);
...
// 完成标记不因冲刷而丢失: 若指令确实完成, 即使冲刷周期到达也应记录 done;
// 被冲刷的年轻指令 rob_valid 会同步清零, 残留 done 不会导致错误提交。
// 但有两类完成必须丢弃:
//   1. flush 拍到达的年轻完成 (cmt_washed_1/2);
//   2. flush 后一拍到达的完成: 其来自 flush 前拍寄存的 CDB, 携带 flush 前
//      的旧 id。rename 在 flush 时回卷 id_pool, 重放的新指令会拿到相同的
//      id 值 → 旧完成会错误地标记新指令 done 并写入旧 bus → 提前提交、
//      结果错误 (如 min3 的 a4=0)。故需保持一拍屏蔽 (flush_hold)。
reg        flush_hold;
reg [ID_WIDTH-1:0] flush_id_hold;
always @(posedge clk) begin
    if (rst) begin
        flush_hold    <= 1'b0;
        flush_id_hold <= {ID_WIDTH{1'b0}};
    end else if (flush) begin
        flush_hold    <= 1'b1;
        flush_id_hold <= flush_id;
    end else begin
        flush_hold    <= 1'b0;
    end
end
wire cmt_washed_1_hold = flush_hold && (
    (flush_id_hold[ID_WIDTH-1] ^ commit_id_1[ID_WIDTH-1]) ?
    (flush_id_hold[ID_WIDTH-2:0] > commit_id_1[ID_WIDTH-2:0]) :
    (flush_id_hold[ID_WIDTH-2:0] < commit_id_1[ID_WIDTH-2:0]));
wire cmt_washed_2_hold = flush_hold && (
    (flush_id_hold[ID_WIDTH-1] ^ commit_id_2[ID_WIDTH-1]) ?
    (flush_id_hold[ID_WIDTH-2:0] > commit_id_2[ID_WIDTH-2:0]) :
    (flush_id_hold[ID_WIDTH-2:0] < commit_id_2[ID_WIDTH-2:0]));
wire do_commit_1 = commit_valid_1 & ~cmt_washed_1 & ~cmt_washed_1_hold;
wire do_commit_2 = commit_valid_2 & ~cmt_washed_2 & ~cmt_washed_2_hold;
wire do_write_1  = rename_valid_1 && !rob_full;
wire do_write_2  = rename_valid_2 && !rob_full;

assign rob_full = &rob_valid;

assign rob_oldest_valid = rob_valid[rob_tail];
assign rob_oldest_id    = rob_id_buf[rob_tail];
    ...
    if (rst) begin
        ...
        if (flush) begin
            for (i = 0; i < ROB_DEPTH; i = i + 1) begin
                if (rob_valid[i]) begin
                    // ID 比较 + 位置掩码双保险，避免某种情况下年轻指令未被清掉。
                    if (flush_clear_mask[i] ||
                        ((flush_id[ID_WIDTH-1] ^ rob_id_buf[i][ID_WIDTH-1]) ?
        ...
        if ((do_write_1 | do_write_2) & ~flush) begin
            if (do_write_1) begin
                ...
                rob_done[rob_head]   <= 1'b0;   // 分配必须清 done:
                                                // flush 拍年轻指令的完成会覆盖 flush 的清零,
                                                // 残留 done=1 会让新指令未完成即提交 (陈旧 bus)
            end
            if (do_write_2) begin
                rob_id_buf[rob_head + {{ROB_PTR_WIDTH-1{1'b0}}, do_write_1}] <= rename_id_2;
                rob_valid[rob_head + {{ROB_PTR_WIDTH-1{1'b0}}, do_write_1}]  <= 1'b1;
                rob_done[rob_head + {{ROB_PTR_WIDTH-1{1'b0}}, do_write_1}]   <= 1'b0;
            end
