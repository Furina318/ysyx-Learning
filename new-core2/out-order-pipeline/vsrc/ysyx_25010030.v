...
`endif
    ...
    // ID_WIDTH 需覆盖"flush 后保留的旧条目 + 新指令"的 id 空间:
    // 6 位 (64) 在 flush 保留较多旧条目时, 新指令 id 回绕后与旧条目冲突
    // (commit_match 错乱 → 指令流错位)。7 位 (128) > 2*ROB_DEPTH, 保证
    // 环形比较的 id 距离恒小于半空间, 回绕不再与在飞条目冲突。
    localparam ID_WIDTH      = $clog2(RENAME_DEPTH) + 2;
    ...
    // ── perf counters output from issue (方案A) ──
    wire [47:0] iq_occup_acc, iq_cyc_acc, iq_full_acc, iq_dispatch_acc;
    wire [47:0] iq_issue_slot1_acc, iq_issue_slot2_acc;
    wire [47:0] iq_issue_quick_acc, iq_issue_mdu_acc, iq_issue_lsu_acc;
    wire [47:0] iq_issue_lsu_load_acc, iq_issue_lsu_store_acc;
    wire [47:0] iq_issue_ctrl_acc, iq_issue_special_acc;

    wire alu1_free, alu2_free, mdu_free, lsu_free, lsu_load_free, bru_free, csr_free;
    wire        rob_oldest_valid;
    wire [ID_WIDTH-1:0] rob_oldest_id;
    ...
    wire [31:0] bru_pc, bru_dnpc, bru_pred_dnpc;
    wire        bru_dnpc_valid, bru_taken;
    wire        bru_is_call, bru_is_ret, bru_is_jal, bru_is_jalr, bru_is_indirect, bru_is_c_inst;
    // 执行端反馈: 该指令预测时刻的元数据快照 (供 BPU 训练/恢复)
    wire [31:0] bru_ghr;
    wire [ 2:0] bru_provider;
    wire        bru_alt_taken, bru_dir;
    wire [ 3:0] bru_ras_ptr;
    wire [ 4:0] bru_ras_cnt;
    // 取指端: 预测时刻元数据快照 (随指令携带)
    wire [31:0] bpu_ghr1, bpu_ghr2;
    wire [ 2:0] bpu_prov1, bpu_prov2;
    wire        bpu_alt1, bpu_alt2, bpu_dir1, bpu_dir2;
    wire [ 3:0] bpu_ras_ptr1, bpu_ras_ptr2;
    wire [ 4:0] bpu_ras_cnt1, bpu_ras_cnt2;
    ...
    // 非阻塞 dcache 接口 (MSHR 完成端口)
    wire        dcache_hit_o;
    wire        dcache_ret_valid;
    wire [ID_WIDTH-1:0] dcache_ret_tag;
    wire [31:0] dcache_ret_data;
    wire        lsu_ret_ready;
    wire        lsu_head_valid;
    wire [ID_WIDTH-1:0] lsu_head_tag;
    wire        lsu_load_active;
    wire        lsu_hit_w;
    wire        lsu_done_valid;
    wire [31:0] lsu_done_data;
    wire [ 5:0] lsu_done_phy_rd;
    wire [ 4:0] lsu_done_rd;
    wire        lsu_done_gpr_we;
    wire [31:0] lsu_done_pc, lsu_done_inst;
    wire [ID_WIDTH-1:0] lsu_done_id;
    wire [31:0] lsu_done_addr;
    wire        lsu_done_ack;
    wire [ 5:0] lsu_cur_phy_rd;
    wire [31:0] lsu_cur_pc, lsu_cur_inst;
    wire        lsu_cur_gpr_we;
    ...
    bpu u_bpu (
        ...
        .bpu_dir1         (bpu_dir1            ),
        .bpu_dir2         (bpu_dir2            ),
        .bpu_ras_ptr1     (bpu_ras_ptr1        ),
        .bpu_ras_ptr2     (bpu_ras_ptr2        ),
        .bpu_ras_cnt1     (bpu_ras_cnt1        ),
        .bpu_ras_cnt2     (bpu_ras_cnt2        ),
        .pre_way          (pre_way             ), 
        .bpu_en_1         (bpu_en_1            ), 
        .bpu_en_2         (bpu_en_2            ),
        ...
        .bru_ghr_1        (bru_ghr             ),
        .bru_provider_1   (bru_provider        ),
        .bru_alt_taken_1  (bru_alt_taken       ),
        .bru_dir_1        (bru_dir             ),
        .bru_ras_ptr_1    (bru_ras_ptr         ),
        .bru_ras_cnt_1    (bru_ras_cnt         )
    );
    ...
    issue #(
        ...
        .iq_occup_acc            (iq_occup_acc         ),
        .iq_cyc_acc              (iq_cyc_acc           ),
        .iq_full_acc             (iq_full_acc          ),
        .iq_dispatch_acc         (iq_dispatch_acc      ),
        .iq_issue_slot1_acc      (iq_issue_slot1_acc   ),
        .iq_issue_slot2_acc      (iq_issue_slot2_acc   ),
        .iq_issue_quick_acc      (iq_issue_quick_acc   ),
        .iq_issue_mdu_acc        (iq_issue_mdu_acc     ),
        .iq_issue_lsu_acc        (iq_issue_lsu_acc     ),
        .iq_issue_lsu_load_acc   (iq_issue_lsu_load_acc),
        .iq_issue_lsu_store_acc  (iq_issue_lsu_store_acc),
        .iq_issue_ctrl_acc       (iq_issue_ctrl_acc    ),
        .iq_issue_special_acc    (iq_issue_special_acc ),
    ...
    exu_slot1 #(
        ...
        .bru_ghr        (bru_ghr         ),
        .bru_provider   (bru_provider    ),
        .bru_alt_taken  (bru_alt_taken   ),
        .bru_dir        (bru_dir         ),
        .bru_ras_ptr    (bru_ras_ptr     ),
        .bru_ras_cnt    (bru_ras_cnt     ),
    ...
    exu_slot2 #(
        ...
        .lsu_done_data        (lsu_done_data        ),
        .lsu_done_phy_rd      (lsu_done_phy_rd      ),
        .lsu_done_rd          (lsu_done_rd          ),
        .lsu_done_gpr_we      (lsu_done_gpr_we      ),
        .lsu_done_pc          (lsu_done_pc          ),
        .lsu_done_inst        (lsu_done_inst        ),
        .lsu_done_id          (lsu_done_id          ),
        .lsu_done_addr        (lsu_done_addr        ),
        .lsu_done_ack         (lsu_done_ack         ),

        .lsu_inst_id          (lsu_inst_id          ),  
        .lsu_cur_phy_rd       (lsu_cur_phy_rd       ),
        .lsu_cur_pc           (lsu_cur_pc           ),
        .lsu_cur_inst         (lsu_cur_inst         ),
        .lsu_cur_gpr_we       (lsu_cur_gpr_we       ),
    ...
    lsu #(
        ...
        .dcache_hit           (dcache_hit_o         ),
        .dcache_ret_data      (dcache_ret_data      ),
        .dcache_ret_valid     (dcache_ret_valid     ),
        .dcache_ret_tag       (dcache_ret_tag       ),
        .dcache_ret_ready     (lsu_ret_ready        ),
        .lsu_head_valid       (lsu_head_valid       ),
        .lsu_head_tag         (lsu_head_tag         ),
        .lsu_load_active      (lsu_load_active      ),
        .lsu_tag              (lsu_inst_id          ),
        .lsu_phy_rd           (lsu_cur_phy_rd       ),
        .lsu_pc               (lsu_cur_pc           ),
        .lsu_inst             (lsu_cur_inst         ),
        .lsu_gpr_we           (lsu_cur_gpr_we       ),
        .flush                (flush_once           ),
        .flush_id             (exu_flush_id         ),
        .lsu_hit              (lsu_hit_w            ),
        .lsu_done_valid       (lsu_done_valid       ),
    ...
    dcache u_dcache (   // ID_WIDTH 用模块默认值 7, 保持 Verilator 实例类名稳定
        ...
        .lsu_hit    (dcache_hit_o      ),
        .lsu_tag    (lsu_inst_id       ),
        .lsu_ret_valid (dcache_ret_valid),
        .lsu_ret_tag   (dcache_ret_tag  ),
        .lsu_ret_data  (dcache_ret_data ),
        .lsu_ret_ready (lsu_ret_ready   ),
        .lsu_head_valid (lsu_head_valid  ),
        .lsu_head_tag   (lsu_head_tag    ),
        .lsu_retry      (lsu_load_active ),
        .flush      (flush_once        ),
        .flush_id   (exu_flush_id      )
    ); 
