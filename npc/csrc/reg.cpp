...
void regs_display() 
{
    ...
    _Log(ANSI_FG_YELLOW "$%s\t" ANSI_NONE " 0x%08x\t %010u\n", "pc", 
        top->rootp->rv32e__DOT__IF_ID_pc, top->rootp->rv32e__DOT__IF_ID_pc);
}
...
word_t single_reg_display(char *reg_name) 
{
    ...
    if(strcmp(reg_name, "pc") == 0)
    {
        _Log(ANSI_FG_YELLOW "$%s\t" ANSI_NONE " 0x%08x\t %010u %010d\n", "pc", 
             top->rootp->rv32e__DOT__IF_ID_pc, top->rootp->rv32e__DOT__IF_ID_pc, top->rootp->rv32e__DOT__IF_ID_pc);
        return top->rootp->rv32e__DOT__IF_ID_pc;
    }
