module AXI_ARB (
    ...
    function [1:0] decode_address(input [31:0] addr);
        if (addr >= 32'h8000_0000 && addr <= 32'h87ff_ffff) begin
            decode_address = SRAM;
        end else if (addr >= 32'ha000_0000 && addr <= 32'ha000_0007) begin
            decode_address = UART;
        end else begin
            decode_address = ERR;
        end
    ...
    always @(*) begin
        case (current_master)
            ...
            IFU: begin
                if ((sram_rvalid && sram_rready) || (uart_rvalid && uart_rready)) begin
                    next_master = NONE;
                end else begin
                    next_master = IFU;
                end
            ...
            MEM_WRITE: begin
                if ((sram_bvalid && sram_bready) || (uart_bvalid && uart_bready)) begin
                    next_master = NONE;
                end else begin
