...
module LSU_AXI (
    ...
    always @(posedge clk) begin
        if (rst) begin
        ...
        end else if (lsu_wb_valid & wb_lsu_ready) begin
            l_load    <= 0;
            l_rd_en   <= ex_lsu_RegWrite;
            l_rd_addr <= ex_lsu_rd;
            l_inst    <= ex_lsu_inst;
            ...
            l_MemLen  <= ex_lsu_MemLen;
            l_opcode  <= l_inst[6:0];
        end else if (ex_lsu_valid & lsu_ex_ready) begin // 非访存指令
            l_load    <= l_load;
            l_rd_en   <= ex_lsu_RegWrite;
            l_rd_addr <= ex_lsu_rd;
            l_inst    <= ex_lsu_inst;
    ...
    always @(*) begin
        if (l_load) begin
            rd_data = wr_rd_data;
        end else if (ex_lsu_forward_las) begin
            rd_data = data_in;
        end else if (ex_lsu_MemWrite) begin
            rd_data = 32'h0;
        end else if (ex_lsu_csr & !ex_lsu_csr_ecall & !ex_lsu_csr_mret) begin
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            lsu_wb_valid <= 1;
        end else if (read_valid || write_valid) begin
            lsu_wb_valid <= 1;
        end else if (ex_lsu_valid & lsu_ex_ready & (ex_lsu_MemRead | ex_lsu_MemWrite)) begin
            lsu_wb_valid <= 0;
        end else begin
