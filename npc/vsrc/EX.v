...
module EX (
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            state = next_state;
            if (state == BUSY && id_valid && ex_ready) begin//处于工作状态且握手成功
                case (alu_op)
                    ...
                    `ALU_SLTU: alu_result = (rs1_val < ((opcode[6:2] == `INST_TYPE_R || opcode[6:2] == `INST_TYPE_B) ? rs2_val : imm)) ? 32'b1 : 32'b0;
                    `ALU_SLT:  alu_result = ($signed(rs1_val) < $signed((opcode[6:2] == `INST_TYPE_R || opcode[6:2] == `INST_TYPE_B) ? rs2_val : imm)) ? 32'b1 : 32'b0;
                ...
                ex_valid = 1'b1;//为下游做好准备
            end
            if (state == STALL) begin//和下游握手成功
                ex_valid = 1'b0;
            end
        end
    end
