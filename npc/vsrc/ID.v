...
module ID (
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            state = next_state;
            if (if_valid && id_ready) begin//当if和id握手成功时候
                // 在 IDLE 状态接收新指令并开始译码
                instr_reg = instr;
            end
            if(state == STALL && ex_ready && id_valid) begin
                id_valid = 1'b0; //译码完成
            end
        end
    end
    ...
    always @(*) begin
        case (state)
            ...
            BUSY: begin
                ...
                next_state = ex_ready ? STALL : BUSY;
            end
            STALL: begin
                id_ready = 1'b0;
                id_valid = 1'b1;
    ...
    always @(*) begin
        $display("ID: instr=0x%08x | if_valid=%b | id_ready=%b | id_valid=%b | ex_ready=%b", 
                 instr_reg, if_valid, id_ready, id_valid, ex_ready);
    end
endmodule
