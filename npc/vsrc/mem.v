...
module mem(
    ...
    input WrEn,                // 写使能
    input RdEn,                // 读使能
    input MemtoReg,            // 内存到寄存器选择

    output wire [31:0] inst_data,  // 指令数据输出
    ...
    always @(posedge clk) begin
        if (RdEn || WrEn)
            $display("Mem access: addr = %h, RdEn = %b, WrEn = %b, time = %0t", addr, RdEn, WrEn, $time);
    end

    // 读指令（始终使能，因为指令读取通常是连续的）
    ...
    always @(posedge clk) begin
        ...
        if (WrEn) begin
            case(MemOp)
                ...
                3'b101: begin
                    ...
                    pmem_write(addr, temp_data, 2);             // 2字节写无符号扩展 (特殊情况)
                end 
                3'b100: begin
                    temp_data <= {24'b0, data_in[7:0]};
                    pmem_write(addr, temp_data, 1);             // 1字节写无符号扩展 (特殊情况)
                end 
                default: begin
        ...
        if (RdEn) begin
            if (MemtoReg) begin
                data_out <= addr;  // 实际上是输出ALU的result
            end
            else begin
                case(MemOp)
            ...
            data_out <= data_out;  // 当读使能关闭时保持输出不变
        end
    end
