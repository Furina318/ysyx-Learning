...
module rv32e (
    ...
    wire        stall;          // 数据冒险信号
    wire        flush;          // 控制冒险信号
    wire        struct_hazard;  // 结构冒险信号

    //当MEM模块访问内存的时候，暂停IF模块
    assign struct_hazard = (ex_mem_MemRead || ex_mem_MemWrite) && if_ready;

    //前递信号
    wire forwardA_EX, forwardB_EX, forwardA_MEM, forwardB_MEM;

    // EX/MEM到ID的前递
    assign forwardA_EX = (id_rs1 == ex_mem_rd) && ex_mem_RegWrite && (id_rs1 != 5'b0) && ex_mem_valid;
    assign forwardB_EX = (id_rs2 == ex_mem_rd) && ex_mem_RegWrite && (id_rs2 != 5'b0) && ex_mem_valid;
    // MEM/WB到ID的前递
    assign forwardA_MEM = (id_rs1 == mem_wb_rd) && mem_wb_RegWrite && (id_rs1 != 5'b0) && mem_wb_valid;
    assign forwardB_MEM = (id_rs2 == mem_wb_rd) && mem_wb_RegWrite && (id_rs2 != 5'b0) && mem_wb_valid;

    // 选择前递信号
    assign id_rs1_val = forwardA_EX ? ex_mem_alu_result :
                        (forwardA_MEM ? wb_data : rs1_val);
    assign id_rs2_val = forwardB_EX ? ex_mem_alu_result :
                        (forwardB_MEM ? wb_data : rs2_val);
    ...
    always @(posedge clk) begin    
        ...
        if(stall) begin
            $display("\033[32m         [STALL]暂停!!!\033[0m");
        end
        if(struct_hazard) begin
            $display("\033[32m         [STRUCT]IF和MEM访问冲突,结构冒险触发\033[0m");
        end
    end
endmodule